...
export class PokemonViewerElement extends HTMLElement {
  #state = store(
    {
      ...
      typeNames: [] as string[], // Localized type names
      loading: false,
      showLoader: false,
  ...
  connectedCallback() {
    ...
    const renderType = (type: string) => html`
              ...
              <div class="types">
                ${computed(() =>
                  this.#state.typeNames.length > 0
                    ? this.#state.typeNames.map((name) => renderType(name))
                    : (this.#state.pokemon?.types ?? []).map((t) =>
                        renderType(t.type.name),
                      ),
                )}
              </div>
                              ...
                              <div class="types">
                                ${computed(() =>
                                  this.#state.compareTypeNames.length > 0
                                    ? this.#state.compareTypeNames.map((name) =>
                                        renderType(name),
                                      )
                                    : (
                                        this.#state.comparePokemon?.types ?? []
                                      ).map((t) => renderType(t.type.name)),
                                )}
                              </div>
                            `
  ...
  async fetchLocalizedNames(pokemon: Pokemon, isCompare: boolean) {
    ...
    try {
      ...
      if (isCompare) {
        ...
        this.#state.compareTypeNames = pokemon.types.map((t) => t.type.name);
      } else {
        this.#state.pokemonName = pokemon.name;
        this.#state.typeNames = pokemon.types.map((t) => t.type.name);
      }
    }
  }
